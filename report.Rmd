...
The goal of the project is to acquire and analyse swimming event results for the Olympic Games. After exploring multiple data sources, [fina.org](https://www.fina.org) was scraped for its posted swim results which, in addition to the Olympic Games, cover the World Championships, World Cup, Junior Chamionships, Champions SEries, Youth Olympic Games, and Marathon Series. The data was wrangled into a usable format. Among other things, missing splits were added and duplicate results were removed. The data was then explored generally and a deeper dive or "Case Study" was done on the available splits for the 400m Freestyle.
...
- [sports-reference.com](https://www.sports-reference.com) has been used to source Olympic datasets in the past, for example the [Kaggle Olympic Dataset](https://www.kaggle.com/heesoo37/120-years-of-olympic-history-athletes-and-results). As of Summer 2019, most Olympic swimming results prior to 2012 are missing entirely, so it didn't serve well for this project.
...
```{r, duplicate_probabilities, echo = FALSE, dpi = 200}
...
```{r, duplicate_heat_sizes, echo = FALSE, dpi = 200, message = FALSE}
phases %>% 
...
```{r unfaceted_result_density, dpi = 200}
ws %>% 
...
```{r faceted_result_density, dpi = 200}
ws %>% 
  filter(status == "TIME", distance == 200, style == "Freestyle", !relay) %>%
  ggplot(aes(time, stat(count))) +
  geom_density() + 
...
```{r unfaceted_400free_splits, dpi = 200}
cs_style <- "Freestyle"
cs_distance <- 400
cs_splits <- splits %>% 
  inner_join(ws, by = "result_id") %>% 
...
```{r adjusted_400free_splits, dpi = 200}
cs_ratios <- cs_splits %>% 
...
```{r correlation_plot, dpi = 200, echo = FALSE}
cs_matrix <- cs_ratios %>% 
  select(ratio, leg, result_id) %>%
  pivot_wider(names_from = leg, values_from = ratio, names_prefix = "Leg ") %>%
  column_to_rownames(var = "result_id")
cs_cor <- cor(cs_matrix)
corrplot(cs_cor, type = "lower", tl.col = "black", 
         tl.srt = 45)
```
...
```{r split1_lm_plot, dpi = 200, echo = FALSE}
# Plot Regression Lines against data
cs_test %>%
  ggplot(aes(leg1, time)) + 
  geom_point(alpha = .5) + 
  geom_smooth(method=lm, se = FALSE, color = "purple") +
  theme + 
  labs(x = "First Split (s)", y = "Final Time (s)")
```

The first split is significantly predictive as expected - faster swimmers begin the race faster - so the reduction in RMSE was more than 50%, from `r rmse0` to `r rmse1` for the linear regression. 
...
```{r kendall_tau_distribution, dpi = 200, echo = FALSE}
not_summary_results <- ws %>%
  filter(!is_summary) %>%
  select(result_id, phase_id)

cs_kt <- cs_obs %>%
  inner_join(not_summary_results, by = "result_id") %>%
  mutate(
    pred1 = predict.train(fit1, newdata = .),
    pred2 = predict.train(fit2_lm, newdata = .),
    pred4 = predict.train(fit4, newdata = .)
  ) %>%
  group_by(phase_id) %>%
  summarise(
    acc1 = kendall.tau(time, pred1),
    acc2 = kendall.tau(time, pred2),
    acc4 = kendall.tau(time, pred4)
  ) %>% 
  pivot_longer(cols = starts_with("acc"),
               names_to = "legs",
               names_prefix = "acc",
               values_to = "accuracy") %>%
  mutate(legs = as.integer(legs))
...
cs_firsts <- cs_obs %>%
  ...
  group_by(phase_id) %>%
  summarise(
    win = result_id[order(time)[1]],
    win1 = result_id[order(pred1)[1]] == win,
    win2 = result_id[order(pred2)[1]] == win,
    win4 = result_id[order(pred4)[1]] == win
  ) %>% select(-win) %>%
  pivot_longer(cols = starts_with("win"),
               names_to = "legs",
               names_prefix = "win",
               values_to = "hit") %>%
  group_by(legs) %>%
...
cs_kt %>% 
  ggplot(aes(x = legs, y = accuracy, group = legs)) + 
  geom_point(alpha = .2) + 
  geom_smooth(method = "lm") + 
  geom_boxplot() + 
  labs(x = "Lengths Used", y = "Correlation Coefficient") + 
  theme
```
...
1. Understanding [fina.org](https://www.fina.org) results and scraping with [finaspider.py](/finacrawler/finacrawler/spiders/finaspider.py)
