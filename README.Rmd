...
The k-nearest neighbors and random forest algorithms perform worse on the data than a simple linear regression. The reason for this, I suspect, is that the data is simply best fit by regression rather than other ML algorithms because the predictors and the outcome are all smooth, roughly normally distributed continous variables. 

#### Prediction with Four Splits

Since our linear regression was our strongest model at the 100-meter mark, for 200m we simply do the same:
...
cs_ranks %>% 
  ...
  geom_boxplot() + 
  geom_line(data = cs_firsts, mapping = aes(x = legs, y = accuracy, group = "all")) +
  labs(x = "Lengths Used", y = "Correlation Coefficient") + 
  theme
```

A more intelligible summary of the data is our ability to simply guess the winner. After 50m, 100m, and 200m the model predicts the winner with 48%, 55%, and 68% respectively.
