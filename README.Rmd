...
## Split Case Study: 400m Freestyle

### Exploratory Data Analysis
The split data gives a greatly enriched picture of a race. Here, I take a deeper look at the split data for the 400m Freestlye and how the splits are predictive of one another.
...
library(corrplot)
corrplot(cs_cor, type = "upper", tl.col = "black", 
         tl.srt = 45)
```

The overall trend of the plot is that legs close together are positively correlated and legs further apart are negatively correlated. We already knew this from the previous plot because, since the standard deviation of the middle four lengths is small, deviations from the eventual split average early in the race need to be compensated for at the end of the race. The interesting part about the correlations is that it is not a completely uniform trend. If it were, we would expect the first and last split to be the most negatively correlated but this is not the case - it is the second and second-to-last splits which are most negatively correlated. It seems that the final length is more resilient. Going out very fast (relative to your eventual time) in the initial 100 is normally reflected in slowneww between the 250 and 350 mark.
...
How predictive of final time are a race's initial splits? If we see initial splits, how closely can we predict the final time? Because of the variations due to pool type in gender, this study is further limited to only Women's Long Course 400m Freestyle. With one, two, or four splits, how well can regression and other ML techniques predict the final time? I divide the dataset containing `r n_all` into a training set with 80% of the races and a test set with the other 20%. Our baseline model is just predicting final times in the test set as the average of final times in the train set - our model if we had no splits available and were using no other features. For a measure of accuery I use RMSE:
...
```{r}
# Single Degree Regression
fit1_lm1 <- train(time ~ leg1, data = cs_train, method = "lm")
pred1_lm1 <- predict.train(fit1_lm1, newdata = cs_test)
rmse1_lm1 <- RMSE(pred1_lm1, cs_test$time)
rmse1_lm1

# Two Degree Regression
fit1_lm2 <- train(time ~ poly(leg1, 2), data = cs_train, method = "lm")
pred1_lm2 <- predict.train(fit1_lm2, newdata = cs_test)
rmse1_lm2 <- RMSE(pred1_lm2, cs_test$time)
rmse1_lm2

rmse1 <- max(rmse1_lm1, rmse2_lm2)
...
cs_test %>%
  ...
  theme + 
  labs(x = "First 50 (s)", y = "Final Time (s)")
```

The first split is significantly predictive as expected - faster swimmers begin the race faster - so the reduction in RMSE was more than 50 percent, from `r rmse0` to `r rmse1` for the linear regression. Is the variation mostly coming from the final lap? As in, could we predict accurately up to the 300 or 350 mark, but then the last 50 is too unpredictable? Or is every 50 variable? We can create and visualize a leg-by-leg prediction:



#### Prediction with Two Splits

With a second split we have significantly more to play with because not only do we have the time so far, but the change in pace between the first and second lengths. Thus our model has two features, the first and second splits. Here I represent that information with the first split and it's ratio to the second split:

```{r}
# Linear Regression Model (2 Degrees of Each Variable)
cs_train <- cs_train %>%
  mutate(ratio12 = leg2/leg1)
...
fit2_lm2 <- train(time ~ poly(leg1,2) + poly(ratio12,2) , data = cs_train, method = "lm")
pred2_lm2 <- predict.train(fit2_lm2, newdata = cs_test)
rmse2_lm2 <- RMSE(pred2_lm2, cs_test$time)
rmse2_lm2
...
```{r}
cut_splits <- function(data, n){
  cs_obs %>%
    pivot_longer(cols = starts_with("leg"),
                 names_to = "leg",
                 names_prefix = "leg",
                 values_to = "split") %>%
    filter(leg <= n) %>%
    group_by(result_id) %>%
    mutate(time = sum(split)) %>%
    ungroup() %>%
    pivot_wider(names_from = leg, values_from = split, names_prefix = "leg")
}
...
rmse1_by_leg <- sapply(legs, function(n){
  fit_temp <- train(time ~ poly(leg1,2), cut_splits(cs_train,n), method = "lm")
  test_temp <- cut_splits(cs_test,n)
  prediction_temp <- predict.train(fit_temp, newdata = test_temp)
  RMSE(prediction_temp, test_temp$time)
})
```